...
class PredictionTool:
    ...
    def __init__(
        ...
        challenger: "PredictionTool | None" = None,
    ):
        self.model = model
        # Champion/challenger: the classical model is the champion that drives
        # the answer; the deep model is scored on the same features so every
        # prediction carries a side-by-side comparison instead of the two
        # tracks only being compared once, offline, in a notebook.
    ...
    def _fetch_features_for_customer(self, customer_id: str) -> dict[str, Any]:
        ...
        # Opened read-only, and closed explicitly: `with sqlite3.connect(...)`
        # is a *transaction* context manager, it does NOT close the handle, so
        # the previous version leaked one connection per prediction and could
        # in principle write to the analytics database.
        conn = sqlite3.connect(f"file:{Path(self.db_path)}?mode=ro", uri=True)
        try:
    ...
    def predict(
        ...
        try:
            if features is None:
                if customer_id is None:
                    return PredictionResult(
                        ...
                        status="error",
                        reason=("No customer id found in the question. Ask about a "
                                "specific customer, e.g. 'will customer 12583 buy again?'"),
                    )
            ...
            if self.challenger is not None:
                challenger_result = self.challenger.predict(features=features)
                challenger_payload = {
                    "model_version": challenger_result.model_version,
                    "prediction": challenger_result.prediction,
                    "probability": challenger_result.probability,
                    "label": challenger_result.label,
                    "status": challenger_result.status,
                    "reason": challenger_result.reason,
                }
...
def get_default_prediction_tool(
    ...
    challenger_name: str | None = "dl_repeat_purchase_mlp_v1",
) -> "PredictionTool | None":
    ...
    Loads the classical model as champion and, when the artifact exists, the
    deep MLP as challenger. A missing DL artifact is not an error — the agent
    simply runs champion-only.
    """
    challenger = None
    if challenger_name:
        try:
            challenger = PredictionTool.from_artifact(
                Path(models_dir) / challenger_name, db_path=db_path
            )
        except Exception as exc:
            print(f"[predict] challenger (DL) model unavailable: {exc}")

    try:
        return PredictionTool.from_artifact(
            Path(models_dir) / artifact_name, db_path=db_path, challenger=challenger
        )
    except Exception as exc:
