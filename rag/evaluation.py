...
def build_eval_set(customer_id: str = "12583") -> list[EvalCase]:
    ...
    ``customer_id`` is a parameter because the prediction cases only complete
    when the id actually exists in whichever database the agent is pointed at.
    Hard-coding an id made the prediction rows fail on the sample database and
    dragged task-completion down for a reason that had nothing to do with the
    agent. :func:`evaluate` resolves a real id automatically.
    """
    cid = str(customer_id)
    return [
        # --- RAG -----------------------------------------------------------
        ...
        # --- Prediction ------------------------------------------------------
        EvalCase(f"Predict whether customer {cid} will make a repeat purchase",
                 ["prediction"], "prediction"),
        EvalCase(f"What is the likelihood that customer {cid} churns?",
                 ["prediction"], "prediction"),
        # --- Combined --------------------------------------------------------
        ...
        EvalCase("According to our definition of a high-value customer, list them",
                 ["sql", "rag"], "combined"),
        EvalCase(f"What is the churn probability for customer {cid}, and how is churn defined?",
...
DEFAULT_EVAL_SET: list[EvalCase] = build_eval_set()


def resolve_eval_customer_id(agent, fallback: str = "12583") -> str:
    """Pick a customer id that really exists in the agent's database."""
    tool = getattr(agent, "sql_tool", None)
    if tool is None:
        return fallback
    try:
        result = tool.execute(
            ...
            "GROUP BY o.customer_id ORDER BY COUNT(*) DESC LIMIT 1"
        )
        if result.get("status") == "success" and result.get("rows"):
            return str(result["rows"][0]["customer_id"])
    except Exception:
        pass
    return fallback
...
def evaluate(agent, cases: list[EvalCase] | None = None) -> dict:
    ...
    if cases is None:
        cases = build_eval_set(resolve_eval_customer_id(agent))
    rows = []
