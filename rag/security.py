...
# Internal catalogue tables. A SELECT against these is technically read-only but
# it leaks the full schema (and any stored SQL) to whoever is driving the agent,
# so it is treated as an exfiltration attempt. The schema the model is *allowed*
# to see is served by SQLAnalyticsTool.get_schema(), not by generated SQL.
FORBIDDEN_OBJECTS: tuple[str, ...] = (
    "SQLITE_MASTER", "SQLITE_TEMP_MASTER", "SQLITE_SCHEMA",
    "INFORMATION_SCHEMA", "PG_CATALOG", "PG_SHADOW", "MYSQL.USER",
)
...
def validate_sql(
    ...
    # 3b. no catalogue / metadata tables
    for obj in FORBIDDEN_OBJECTS:
        if re.search(rf"\b{re.escape(obj)}\b", scan):
            return {
                "allowed": False,
                "reason": f"Access to internal catalogue object is not allowed: {obj}.",
                "sql": raw,
            }
