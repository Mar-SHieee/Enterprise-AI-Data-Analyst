...
PREDICTION_TOOL_DESCRIPTION = """prediction — Run a trained machine-learning model
to predict a customer's FUTURE behaviour (repeat-purchase likelihood, churn
probability) from a customer_id. Both a classical model (Member 3) and a deep
MLP (Member 4) are available behind this tool.
Use it when the question asks to predict, forecast or estimate the likelihood
of something happening — not for historical facts or aggregates (use sql) and
not for definitions or policy questions (use rag). If the question asks for a
prediction *and* what a term means, use both prediction and rag."""
