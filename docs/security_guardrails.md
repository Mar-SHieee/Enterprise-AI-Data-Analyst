...
| T9 | Schema exfiltration via a read-only query on the catalogue | Attacker maps the database before probing it | catalogue deny-list in `validate_sql` |
| T10 | Unbounded question used to flood the prompt / context | Cost blow-up, prompt stuffing | `MAX_QUESTION_LENGTH` enforced in `check_user_input` |
...
- catalogue objects are blocked (`sqlite_master`, `sqlite_schema`,
  `information_schema`, `pg_catalog`): a `SELECT` against them is technically read-only but
  it dumps the whole schema, and the schema the model is allowed to see is served by
  `SQLAnalyticsTool.get_schema()`, not by generated SQL;
